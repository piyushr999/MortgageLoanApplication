...
public class UserInputValidator {
	...
	public boolean validateInput(UserInputDto userInput, Double price) throws InvalidInputException {
		if (Period.between(userInput.getDateOfBirth(), LocalDate.now()).getYears() < 25) {
			throw new InvalidInputException("You are not allowd for loan. Minimum age should be 25");
		} else if (userInput.getMonthlySalary() < 10000) {
			throw new InvalidInputException("You are not allowd for loan. Minimum salary should be 10000");
		} else if ((userInput.getPropAreaSize() * price) < 500000) {
			throw new InvalidInputException("You are not allowd for loan. Minimum property value should be 500000");
		}
		return true;
	}
}
